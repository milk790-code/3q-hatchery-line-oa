...
const DEFAULT_INPUT = path.join(ROOT, "data", "funnel_aggregates.example.csv");
const DEFAULT_OUTPUT = path.join(ROOT, "data", "funnel_aggregates.preview.jsonl");
const STATUS_PATH = process.env.FUNNEL_AGGREGATE_STATUS_PATH
  ? path.resolve(process.env.FUNNEL_AGGREGATE_STATUS_PATH)
  : path.join(ROOT, "data", "funnel_aggregate_status.json");
const REAL_EVENTS_PATH = process.env.FUNNEL_REAL_EVENTS_PATH
  ? path.resolve(process.env.FUNNEL_REAL_EVENTS_PATH)
...
const REQUIRED_COLUMNS = ["date", "asset_id", "event_type", "count", "content_id", "variant_id"];
...
async function main() {
  ...
  if (applyToRealEvents) {
    ...
    if (exampleCheck.detected) {
      await writeStatus({
        ...blockedStatus(inputPath, outputPath, `Example/template aggregate CSV cannot be applied to data/lp_events.jsonl: ${exampleCheck.reason}.`),
        confirm_real_data: Boolean(options.confirmRealData),
        example_input_detected: true,
      });
      console.error("Blocked: example/template aggregate CSV cannot be applied to real events.");
      process.exitCode = 2;
      return;
    }
  }
  ...
  if (!headerValidation.ok) {
    await writeStatus({
      ...blockedStatus(inputPath, outputPath, headerValidation.error),
      contains_sensitive_columns: headerValidation.containsSensitiveColumns,
    });
    console.error(headerValidation.error);
    process.exitCode = 1;
    return;
  }

  const aggregateRows = parsed.rows.map((row, index) => normalizeAggregateRow(row, index + 2, knownAssetIds));
  ...
  const status = {
    ...
    mode: applyToRealEvents ? "applied_local_events" : "full_funnel_preview",
    ...
    counts_by_asset: countBy(events, "asset_id"),
    counts_by_content_id: countBy(events, "content_id"),
    ...
    note: applyToRealEvents
      ? "Local append only. No external send, deploy, public link change, customer mutation, payment, or deletion."
      : "Preview only. Full-funnel aggregate rows are not scored until explicitly applied to data/lp_events.jsonl.",
  };
...
function normalizeAggregateRow(row, lineNumber, knownAssetIds) {
  const assetId = requiredValue(row.asset_id, "asset_id", lineNumber);
  if (!knownAssetIds.has(assetId)) {
    throw new Error(`CSV line ${lineNumber}: unknown asset_id ${assetId}.`);
  }
  ...
  const cleaned = {
    ...
    count,
    source: optionalValue(row.source) ?? "manual_review",
    medium: optionalValue(row.medium) ?? "full_funnel_aggregate",
    campaign: optionalValue(row.campaign),
    content_id: requiredValue(row.content_id, "content_id", lineNumber),
    variant_id: requiredValue(row.variant_id, "variant_id", lineNumber),
...
function expandAggregateRow(row) {
  ...
  for (let index = 0; index < row.count; index += 1) {
    const event = {
      event_id: `funnel:${hashEvent(row, index)}`,
      ...
      metadata_json: {
        import_source: "full_funnel_aggregate",
        aggregate_only: true,
        no_pii: true,
      },
    };
...
function isSensitiveKey(key) {
  ...
  return SENSITIVE_COLUMN_PATTERNS.some((pattern) => normalized.includes(pattern.replace(/[^a-z0-9_]/g, "")));
}
...
async function detectExampleInput(inputPath, raw) {
  ...
  try {
    ...
    if (normalizeCsvForCompare(raw) === normalizeCsvForCompare(exampleRaw)) {
      return { detected: true, reason: "input content matches data/funnel_aggregates.example.csv" };
    }
  } catch {
