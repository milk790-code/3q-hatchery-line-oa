...
const WORKSHEET_STATUS_PATH = path.join(ROOT, "data", "sample_gate_owner_worksheet_status.json");
const HTML_PATH = path.join(ROOT, "sample_gate_owner_form.html");
const STATUS_PATH = path.join(ROOT, "data", "sample_gate_owner_form_status.json");
...
async function main() {
  ...
  const worksheet = await readOptionalJson(WORKSHEET_STATUS_PATH, {});
  ...
  const status = {
    ...
    status: ownerFilledExists ? "owner_filled_ledger_detected_review_before_overwrite" : "ready_local_browser_fill",
    form_path: HTML_PATH,
    status_path: STATUS_PATH,
    template_path: TEMPLATE_PATH,
    owner_filled_path: OWNER_FILLED_PATH,
    owner_filled_exists: ownerFilledExists,
    worksheet_status: worksheet.status ?? "unknown",
    row_count: rows.length,
    link_count: new Set(rows.map((row) => row.tracking_link_id)).size,
    source_group_count: new Set(rows.map((row) => row.source_group)).size,
    ...
    json_download_filename: "sample_gate_owner_form.review.json",
    browser_only: true,
...
function decorateRow(row, rowNumber) {
  ...
  return {
    ...
    pii_rule: stage === "line_add"
      ? "Aggregate only. Do not paste LINE user IDs, names, chat text, notes, or customer rows."
      : "Aggregate only. Do not paste IP, User-Agent, session rows, or visitor identifiers.",
  };
}
...
function compactStatus(status) {
  return {
    ...
    worksheet_status: status.worksheet_status,
...
<head>
  ...
  <title>3Q Growth Loop Sample Gate Owner Form</title>
  <style>
    :root {
      --ink: #17211b;
      --muted: #66736c;
    ...
    * { box-sizing: border-box; }
    ...
    table { width: 100%; border-collapse: collapse; min-width: 1320px; background: #fff; }
...
<body>
  <main class="shell">
    <section class="top">
      <div>
        <h1>3Q Growth Loop Sample Gate Owner Form</h1>
        <p>BLUF: Fill aggregate counts only, validate locally, then download <span class="mono">sample_gate_ledger.filled.csv</span>. This page has no network calls, no external links, no browser storage, and no event writes.</p>
      </div>
      <aside class="panel">
        <header>
        ...
        <div class="body facts">
          ...
          <div class="fact"><span class="label">Download</span><span class="value">sample_gate_ledger.filled.csv</span></div>
          <div class="fact"><span class="label">Owner path</span><span class="value">data/source_capture/sample_gate_ledger.filled.csv</span></div>
        </div>
      </aside>
    </section>
    ...
    <section class="panel">
      ...
      <div class="body">
        ...
        <div class="scroll">
          <form id="ownerForm" action="about:blank" method="dialog" novalidate>
            <table>
              <thead>
                <tr>
                  <th>CSV row</th>
    ...
    <section class="panel" style="margin-top:14px">
      ...
      <div class="body">
        <ul class="rules">
          ...
          <li>The page only builds a downloadable CSV. It does not create <span class="mono">data/source_capture/sample_gate_ledger.filled.csv</span> by itself.</li>
          <li>After review, place the downloaded CSV at <span class="mono">data/source_capture/sample_gate_ledger.filled.csv</span>, then run <span class="mono">npm run source:compile -- --input=data/source_capture/sample_gate_ledger.filled.csv --input-kind=sample_gate_filled</span>.</li>
          <li>Sample-insufficient rounds keep the champion and do not promote challengers.</li>
        </ul>
      </div>
    </section>
  </main>
  ...
  <script>
    ...
    function render() {
      ...
      rows.forEach((row, index) => {
        ...
        tr.innerHTML = [
          ...
          cell(row.stage_label + "<br><span class='mono'>" + escapeHtml(row.stage) + "</span>"),
    ...
    downloadJson.addEventListener("click", () => {
      ...
      download(contract.json_download_filename, JSON.stringify({
        ...
        mode: "sample_gate_owner_form_review",
