#!/usr/bin/env bash
# 3Q Hatchery — LINE Rich Menu 一鍵建立 + 上傳腳本
# ─────────────────────────────────────────────────────────────────────────
# 用途：
...
# 共用版型：3 cols × 2 rows = 6 cells；hero 跨左上 2 cell (col 1-2 row 1)
...
read -r -d '' SCHEMA_A <<'JSON' || true
{
  ...
  "areas": [
    { "bounds": { "x": 0,    "y": 0,    "width": 1666, "height": 843 },
      "action": { "type": "message", "text": "說說我的店" } },
...
# ── 3) 跑三個 ────────────────────────────────────────────────────────────
ID_A=$(upload_one "A · NEW"        "$SCHEMA_A" "3Q-HATCHERY_richmenu_a_2500x1686.png")
ID_B=$(upload_one "B · INQUIRED"   "$SCHEMA_B" "3Q-HATCHERY_richmenu_b_2500x1686.png")
ID_C=$(upload_one "C · CONVERTED"  "$SCHEMA_C" "3Q-HATCHERY_richmenu_c_2500x1686.png")

# ── 4) A 版設為新好友預設 ───────────────────────────────────────────────
echo ""
echo "▸ 設定 A 版為預設 rich menu (新好友看到的)…"
...
═══════════════════════════════════════════════════════════════════════
  ✓ 三版 Rich Menu 都建好了。把下面 3 行貼到 worker.js：
═══════════════════════════════════════════════════════════════════════
...
═══════════════════════════════════════════════════════════════════════
  ...
  • Worker 觸發 INQUIRED 切換 → linkRichMenu(\$userId, RICHMENU_INQUIRED)
  • Worker 觸發 CONVERTED 切換 → linkRichMenu(\$userId, RICHMENU_CONVERTED)
═══════════════════════════════════════════════════════════════════════
EOF
