...
// ═══ POP /go 三問式免費成果採集(v1)═══
var GO_INTAKE_ALLOWED_SOURCES = new Set(["direct", "business-card", "package-insert", "social", "legacy-worker"]);
var GO_INTAKE_FB_SOURCE = /^fb-[0-9a-f]{6}$/;
...
var GO_INTAKE_SERVICE = Object.freeze({
  ...
  questions: Object.freeze([
    "第 1／3 題｜請貼物件頁或大概位置；門牌可先遮掉號碼。",
    "第 2／3 題｜你預計何時看屋、簽約或入住？",
    "第 3／3 題｜最在意噪音、漏水、治安、交通，還是合約條款？"
  ]),
  delivery: "我會回一頁公開資訊摘要與你看屋、簽約時要追問的清單。",
  boundary: "每人一次、一個地址或物件；僅整理公開資訊與風險線索，不保證物件安全、不代替現勘或法律意見。"
});
...
__name(goIntakeQuestion, "goIntakeQuestion");
...
__name(goIntakeTransition, "goIntakeTransition");
...
__name(goIntakeHealthResponse, "goIntakeHealthResponse");
...
async function handleEvents(events, env, cfg) {
  ...
  for (const ev of events) {
    try {
      ...
      else if (await maybeHandleGoIntake(ev, env, cfg)) continue;
