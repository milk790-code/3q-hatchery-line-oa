...
【v4 情緒價值引擎(你贏過人類業務的地方)】戰術同理:先說出你讀到的他的處境與感受(「你會這樣問,是不是怕簽下去才發現踩雷…」),被理解的人才聽你說。校準提問:用「怎麼/什麼/多少」開頭的開放問句挖需求,不用「是不是」逼選邊,一次只問一個。鏡像:偶爾重複客人最後幾個字,像鄰里長輩接話讓他自己說下去。情緒先於資訊:他焦慮先安撫再給方案,先處理心情再處理事情。嫌貴先問「是跟什麼比呢?」再把報告費翻譯成「守住幾百萬決定的保險費」。永遠站他那邊:連「這個物件不用花錢看,明顯不行」都敢說——願意幫客人省錢,客人反而更信。
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/health') return json({ ok: true, worker: 'tudigong-sales-ai', version: '1.0', brain: env.ANTHROPIC_API_KEY ? 'claude-sonnet-4-6' : (env.AI ? 'workers-ai-llama-3.3-70b' : 'none'), kv: Boolean(env.SESSION), d1: Boolean(env.CRM) });
    ...
    if (url.pathname === '/chat' && request.method === 'POST') {
      ...
      if (env.SESSION) {
        ...
        const rl = `rl:tudiai:${rlIp}:${Math.floor(Date.now() / 60000)}`;
...
(function(){
  ...
  var ACC="#e8632c", BRAND="呆丸土地公", GREET="來,我是土地公的助手。你在看房、租屋、還是開店選點?";
