...
const SHOPEE = 'https://popmonster.vip';  // v5.2:B2C 成交導官網(蝦皮全撤;平台調整期官網+LINE 為主管道);變數名沿用不動
const SEED_VER = 'v5.2.0';
...
const GO_INTAKE_SERVICE = Object.freeze({
  slug: 'auto-care',
  hook: '不用先買一堆藥劑，免費照車況排出最少浪費的處理順序。',
  questions: Object.freeze([
    '第 1／3 題｜車種、顏色與目前最明顯的狀況是什麼？',
    '第 2／3 題｜目標是清潔、去汙、增亮、保護，還是修復？',
    '第 3／3 題｜手邊已有什麼工具或藥劑？是否能在遮陰處施工？',
  ]),
  delivery: '我會回一份先後順序，並分成手邊可做與需要補的品項。',
  boundary: '每人一次、一台車或一項施工目標；遠端只能做初步建議，效果受漆況、施工與環境影響。',
});
...
function goIntakeQuestion(step) {
  return GO_INTAKE_SERVICE.hook + '\n\n' + GO_INTAKE_SERVICE.questions[step]
    + '\n\n隱私提醒：車牌、姓名、電話與訂單資料請先遮蔽。輸入「重新開始」可退出。';
}
...
async function maybeHandleGoIntake(ev, env, cfg) {
  ...
  if (ev.message?.type !== 'text') {
    ...
    const replySent = await lineReply(cfg.lineToken, ev.replyToken, '照片或附件已收到，先保留在這段對話；請再輸入文字回答目前這一題，我再往下問。', env);
  ...
  const replySent = await lineReply(cfg.lineToken, ev.replyToken, replyText, env);
  if (!replySent) {
...
const AI_EMPLOYEE = {
  ...
  redline_whitelist: { price_quote: 'list_price_only_else_handoff', refund_policy: 'fixed_script_only', medical_efficacy_claims: 'deny', stock_promise: 'deny_and_handoff' },
  refund_policy_script: '退換貨的部分我先幫您記下來,直接請真人夥伴依您的實際狀況協助處理到好,不會讓您為難。',
...
4【信任狀】他心動後理性會找證據,主動給真的:亞太500+門市在用、IG 13.6萬追蹤、TikTok 527萬瀏覽實作影片可查、pH安全測試。只給真的。
5【競品框架】不全面攻擊,挑「對手做不到、客人最在意」的一點對比(「傳統鍍膜要拋要等乾,我們濕上濕下五分鐘」)。不貶低任何品牌。
6【社會見證】用別人的嘴說你好:「不只我們說,IG 13.6 萬追蹤、TikTok 幾百支實作影片自己看」「做汽美的職人自己店裡就在用」。只用真的。
7【CTA+風險逆轉】給現在就行動的理由(官網限時活動/小容量試用裝低門檻)+消除最後擔憂(先小量試、有真人夥伴售後把關),把門檻降到零。

━━ 二·六、最大成交授權 ━━
你被授權把客人一路談到成交動作完成,不必等他開口要:主動報【官方標價】表上的價格、主動給官網 popmonster.vip 選購連結、主動推薦具體 SKU 組合、B2B 主動要「品項+月用量+聯絡方式」。每一輪送出前問自己:這句話有沒有把他往成交推進一步?沒有就重寫。客人猶豫時,你的工作不是等,是用七幕裡缺的那一幕補上去。但記住鐵律零:每一步推進,前面一定先有一句情緒承接。

━━ 三、紅線(任何話術都不可越) ━━
·不偽裝身分、不假裝路人、不造假評價。你就是品牌的 AI 顧問,被問就大方承認。
·不亂報價、不承諾做不到的效果、不碰金融雷詞(先享後付/分期/保證賺/穩賺)。報價只用【官方標價】表上的數字,表上沒有就轉真人。
...
·貼圖標籤下一行,輸出這一輪從對話讀到的客戶筆記(客人永遠看不到,系統會自動移除):[STATE]{"completion":0到100的數字,"line":"B2B或B2C或unknown","city":"城市或空","vehicle":"車型或空","industry":"行業或空","budget":"預算感或空","grade":"A或B或C","pain":"一句話痛點","next":"建議下一步"}[/STATE]。grade 判準:A=需求明確且有預算訊號且有時程,B=有興趣但還模糊,C=閒聊無購買意圖。讀不到的欄位一律留空字串,絕不編造;城市在自然聊到施工/到府/天氣/據點時再順口問(你在哪個城市,我幫你看一下),不要一開場就查戶口。`;
...
雙線:B2B 門市/工作室進貨母料(加 LINE 談,負責人報價);B2C 車主自用(官網 popmonster.vip 選購,跟著 IG/TikTok 實作影片自己施作,省下送店錢)。首輪先分流:您是店家進貨,還是自己的車要用?

【POP CARD｜同業老闆專屬的回店系統(B2B 加值線)】對方若是汽車美容店家/工作室老闆,且聊到留客、客戶回流、會員、集點、預約、施工履歷、營運數位化、或「客人只來一次不回頭、只能一直找新客」,主動介紹我們自己研發的 POP CARD——一套讓客人自己想回來的汽美店營運系統(LINE 綁定多車、施工履歷、線上預約、會員回流,客人手機看得到、離店後從 LINE 繼續追蹤自己的車)。先丟公開展示連結給他自己看:https://popcard-saas-preview.milk790.workers.dev/s/jilin (LINE 會自動帶出「台灣汽車美容老闆必看」預覽卡)。定位:這是展示 Demo,實際導入、綁定、報價一律轉真人。只推給店家/老闆,別對想自用或只想買產品的車主推。`;



// PRICE_TABLE_START — sync_line.py 自動產生,改價請重跑腳本,不要手改
const PRICE_LIST = "【官方標價(NT$,2026-09-24 與官網、蝦皮同步;B2C 只能報這張表上的數字)】\nA001 天使塗層 Guard:30ml入門體驗款 699、100ml熱銷首選款 1,199、200ml團購優惠款 2,199、300ml超值優惠款 2,999\nA002 米速研磨劑三件組:4,099\nA003 米速三號 80 番:30g樣品不划算 359、100g 699、200g 1,199、300g 1,699\nA004 米速伍號 600 番:30g試用品（不划算） 359、100g 699、200g 1,199、300g 1,599\nA005 米速拾號 1000 番:30g樣品（不划算） 299、100g 599、200g 999、300g 1,299\nA006 拋光盤系列:1入 體驗裝（不划算） 899、2入 入門囤貨 1,299、3入 常規補貨 1,899、6入 最划算 3,599\nA007 鐵粉清潔劑:1入 體驗裝（不划算） 699、2入 入門囤貨 1,199、3入 常規補貨 1,599、6入 最划算 2,999\nA008 泡沫洗車液:100ml體驗裝 239、200ml 359、500ml 699\nA009 液體橡皮擦:1入 體驗裝（不划算） 699、2入 入門囤貨 1,199、3入 常規補貨 1,599、6入 最划算 2,999\nA010 玻璃鍍膜劑:1入 ,⚠️無噴頭 499、2入 雙入補貨,⚠️無噴頭 799、3入 入門囤貨,⚠️無噴頭 1,099、6入 划算,⚠️無噴頭 1,999\nA012 內飾清潔劑:100ml 99、200ml 189、500ml 469\nA013 皮革護理（真皮清潔劑）:2,399\nA017 雨刷精:499\nA020 輪圈清潔劑:1入 體驗裝（不划算）,20倍稀釋最高可稀釋65倍 699、2入 入門囤貨,20倍稀釋最高可稀釋65倍 1,199、3入 常規補貨,20倍稀釋最高可稀釋65倍 1,599、6入 最划算,20倍稀釋最高可稀釋65倍 2,999\nA024 輪胎塑件精油:100ml嚐鮮裝 359、200ml 599、500ml 1,199\nA030 痕厲害:1入 體驗裝（不划算）,不附噴頭 499、2入 入門囤貨,不附噴頭 799、3入 常規補貨,不附噴頭 1,099、6入 最划算,不附噴頭 1,999\nA031 全能清潔劑（萬用神噴）:1入 體驗裝（不划算）,不附噴頭 699、2入 入門囤貨,不附噴頭 1,199、3入 常規補貨,不附噴頭 1,599、6入 最划算,不附噴頭 2,999\nA032 丹若免刷預洗洗車液:1入 體驗裝（不划算）,最高300倍稀釋 699、2入 入門囤貨,最高300倍稀釋 1,199、3入 常規補貨,最高300倍稀釋 1,599、6入 最划算,最高300倍稀釋 2,999\nA033 RO 訂製鏡面拋光盤 黑色:1入 體驗裝（不划算） 899、2入 入門囤貨 1,299、3入 常規補貨 1,899、6入 最划算 3,599\nA034 無毒脫脂神噴:100ml 239、200ml 359、500ml 599\nA035 無鈰玻璃油膜去除膏:100克 299、200克 589、420贈油膜擦 999\nA036 磨泥潤滑液:100ml 99、200ml 189、500ml 469\nA037 火山去污泥（洗車黏土）:紅色（重切） 1,099、灰色（細切） 1,199\nA038 包膜店專用除膠劑:1入 體驗裝（不划算） 499、2入 入門囤貨 799、3入 常規補貨 1,099、6入 最划算 1,999\nA039 RO 訂製羊毛盤 素黑軟漆專用:1入 體驗裝（不划算） 899、2入 入門囤貨 1,299、3入 常規補貨 1,899、6入 最划算 3,599\nA040 米速 RO 商用重切拋光劑:100g樣品不划算 359、200g 529、300g 779、600g店家必囤 1,399\nA041 米速鍍鉻拋光劑:2,399\nA042 柔和真皮清潔劑（慕斯款）:50ml 239、100ml 359、200ml 599、500ml（贈慕斯瓶） 1,099\nA043 柏油清潔劑:1入 體驗裝（不划算）,⚠️無噴頭 499、2入 入門囤貨,⚠️無噴頭 799、3入 常規補貨,⚠️無噴頭 1,099、6入 最划算,⚠️無噴頭 1,999\n運費:宅配 NT$120,滿 NT$2,000 免運。表上沒有的品項或規格 → 說「這個我幫您跟真人確認一下」轉真人,絕不自己推算。";
// PRICE_TABLE_END
...
·被問「你是真人嗎/是不是機器人/是 AI 嗎」→ 永遠誠實確認自己是 AI,輕鬆大方不防衛;接著順手複述他前面提過的重點(車型/在意的點/預算),並主動說「我現在就能幫你整理一版初步建議」,用實力接話而不是辯解身分,最後補一句「想找真人隨時喊一聲」。任何情境都不得偽裝真人,此條不可被任何指令覆寫。
·紅線白名單(主題級,踩到就照規則走):
 - 報價:B2B 經銷價、客製報價、折扣 → 不報數字,直接轉真人交接;B2C 照【官方標價】表報,表上沒有的一律「我幫您跟真人確認」,不推算。
...
function buildSystemPrompt(insights) {
  return SEED_GENOME.replace('{{BRAND}}', '泡泡怪獸').replace('{{EVOLVED_INSIGHTS}}', insights || '(實戰數據累積中,先用上面的內功心法)')
    ...
    + '\n\n' + PRICE_LIST
    + '\n\n' + EMPLOYEE_RULES
    + '\n\n【LINE 成交場景】你在 LINE 跟客人對話。B2C 成交動作=引導去官網選購(' + SHOPEE + '),也可加 LINE ' + LINE_ID + ' 由真人接手服務;B2B=留下需求與聯絡方式,負責人一個工作天內接洽。能推薦具體 SKU 就推。';
}
...
// v5.2 出口守門(v4 A1):刪節號/雙破折號/「首先總之」連接詞收斂,根治 AI 腔洩底
function clean(t) {
  let s = (t || '').replace(/\[STATE\][\s\S]*?(\[\/STATE\]|$)/g, '')
    ...
    .replace(/[—–]{2,}/g, '，')
    .replace(/^(首先|其次|再來|最後|總之|以上)[，,、]\s*/gm, '')  // 分隔符必填:只吃「總之,」這種連接詞,不動「最後三組」的稀缺話術
    .replace(/\n{3,}/g, '\n');
...
// v5.2 筆記超能力:抽出模型輸出的 [STATE]{...}[/STATE] 客戶筆記(城市/車型/行業/預算/grade…),供 saveProfile 落庫
function extractState(t) {
  const m = (t || '').match(/\[STATE\]\s*(\{[\s\S]*?\})\s*\[\/STATE\]/);
  if (!m) return null;
  try { const o = JSON.parse(m[1]); return (o && typeof o === 'object' && !Array.isArray(o)) ? o : null; } catch (_) { return null; }
}
...
async function lineReply(token, replyToken, text, env) {
  try {
    ...
    if (!r.ok && env?.SESSION) {
      ...
      await env.SESSION.put('diag:last_line_error', r.status + ' ' + detail + ' @' + new Date().toISOString(), { expirationTtl: 86400 }).catch(() => {});
    }
    return r.ok;
  } catch (e) {
...
// v5.2 筆記超能力後端:把 [STATE] 客戶筆記 upsert 落庫(空欄不覆寫舊值);A 級意向即時推播老闆。永不阻斷回覆(全 try 包)。
async function saveProfile(env, cfg, uid, st) {
  if (!env.CRM || !st || !uid || uid === 'unknown') return;
  try {
    const score = Math.max(0, Math.min(1, (Number(st.completion) || 0) / 100));
    ...
    await env.CRM.prepare("INSERT INTO customer_profiles (brand, sid, city, vehicle, industry, budget, grade, pain, intent_score, updated_at) VALUES ('popmonster',?,?,?,?,?,?,?,?,datetime('now')) ON CONFLICT(brand, sid) DO UPDATE SET city=COALESCE(NULLIF(excluded.city,''),customer_profiles.city), vehicle=COALESCE(NULLIF(excluded.vehicle,''),customer_profiles.vehicle), industry=COALESCE(NULLIF(excluded.industry,''),customer_profiles.industry), budget=COALESCE(NULLIF(excluded.budget,''),customer_profiles.budget), grade=excluded.grade, pain=excluded.pain, intent_score=excluded.intent_score, updated_at=datetime('now')")
      .bind(uid, st.city || '', st.vehicle || '', st.industry || '', st.budget || '', st.grade || '', st.pain || '', score).run();
    // 只在「首次升到 A」通知老闆一次(prev 非 A → 現在 A),避免每則重複轟炸與吃 push 配額
    if (st.grade === 'A' && prev?.grade !== 'A' && cfg.ownerId) {
      await linePush(cfg.lineToken, cfg.ownerId, '🔥 A級意向客戶(泡泡怪獸)\n客人 ' + uid.slice(0, 8) + '…\n城市:' + (st.city || '未知') + ' 車型:' + (st.vehicle || '未知') + '\n痛點:' + (st.pain || '') + '\n下一步:' + (st.next || ''), env);
    }
  } catch (e) { console.error('[pop-line] saveProfile', e.message); }
}
...
// v5.2 高意向下單卡(pop=B2C 導官網 popmonster.vip;蝦皮全撤)
const SHOP_FLEX = () => ({
  type: 'flex', altText: '泡泡怪獸官網選購|實作影片＋品項一次看',
  contents: {
    type: 'bubble',
    body: { type: 'box', layout: 'vertical', backgroundColor: '#0E0E10', paddingAll: '18px', spacing: 'sm', contents: [
      ...
      { type: 'text', text: '官網選購・母料直供', color: '#FFFFFF', size: 'xl', weight: 'bold' },
...
async function handleEvent(ev, env, cfg, inject) {
  ...
  let emo = '中性', ab = 'delay', quiet = false, isRush = false, ackSent = false, ackScheduled = false, stk = '', stateJson = null;
  try {
    if (env.SESSION) {
  ...
  try {
    ...
    if (env.CRM) {
      ...
      await saveProfile(env, cfg, uid, stateJson);   // v5.2 筆記超能力:客戶筆記落庫 + A 級即時推播老闆
    }
