...
// Meta /r → LINE attribution contract. Legacy three-part markers remain valid;
// only the optional fourth segment is treated as a correlation ID.
export function parseGoMarker(text) {
  const value = String(text || '');
  const match = value.match(/【(GO:[^】]+)】/);
  if (!match) return { text: value, marker: null };
  const parts = match[1].split(':');
  if (parts[0] !== 'GO' || parts.length < 3 || !parts[1] || !parts[2]) {
    return { text: value, marker: null };
  }
  const cid = parts[3] && /^[0-9a-f]{8}$/i.test(parts[3])
    ? parts[3].toLowerCase()
    : null;
  return {
...
export async function loadMetaAttribution(env, marker) {
  if (!marker?.cid) return null;
  let fbcFound = false;
  try {
    const raw = await env.CLICK_KV?.get('fbc:' + marker.cid);
    if (raw) {
      try {
        const parsed = JSON.parse(raw);
        fbcFound = typeof parsed === 'string'
          ? parsed.length > 0
          : !!(parsed?.fbc || parsed?.value);
      } catch (_) {
        fbcFound = String(raw).length > 0;
      }
    }
  } catch (_) {}
  return {
...
━━ 二·六、最大成交授權 ━━
你被授權把客人一路談到成交動作完成,不必等他開口要:主動報官方標價、主動給官網 popmonster.vip 選購連結、主動推薦具體 SKU 組合、B2B 主動要「品項+月用量+聯絡方式」。每一輪送出前問自己:這句話有沒有把他往成交推進一步?沒有就重寫。客人猶豫時,你的工作不是等,是用七幕裡缺的那一幕補上去。但記住鐵律零:每一步推進,前面一定先有一句情緒承接。
...
·紅線白名單(主題級,踩到就照規則走):
 - 報價:B2B 經銷價、客製報價、折扣 → 不報數字,直接轉真人交接;B2C 一律「以官網 popmonster.vip 標示價為準」。
...
async function ensureTables(env) {
  ...
  try {
    ...
    await env.CRM.prepare("CREATE TABLE IF NOT EXISTS customer_profiles (brand TEXT, sid TEXT, city TEXT, vehicle TEXT, industry TEXT, budget TEXT, grade TEXT, pain TEXT, intent_score REAL DEFAULT 0, care_last_sent_at INTEGER, nurtured_at INTEGER, nurture_count INTEGER DEFAULT 0, meta_cid TEXT, source TEXT, updated_at TEXT DEFAULT (datetime('now')), PRIMARY KEY(brand, sid))").run();
    ...
    await env.CRM.prepare("ALTER TABLE customer_profiles ADD COLUMN meta_cid TEXT").run().catch(() => {});
    await env.CRM.prepare("ALTER TABLE customer_profiles ADD COLUMN source TEXT").run().catch(() => {});
  } catch (e) { console.error('[pop-line] tables', e.message); }
}

// Phase 1 stores only the anonymous join key. Phase 2 can use meta_cid to
// re-read fbc:{cid} from CLICK_KV; raw fbc is deliberately not copied into D1.
export async function saveMetaAttribution(env, uid, attribution) {
  if (!env.CRM || !uid || uid === 'unknown' || !attribution?.cid) return;
  await env.CRM.prepare("INSERT INTO customer_profiles (brand, sid, meta_cid, source, updated_at) VALUES ('popmonster',?,?,?,datetime('now')) ON CONFLICT(brand, sid) DO UPDATE SET meta_cid=excluded.meta_cid, source=excluded.source, updated_at=datetime('now')")
    .bind(uid, attribution.cid, attribution.source).run();
}
...
async function handleEvent(ev, env, cfg) {
  ...
  const goPayload = parseGoMarker(userMsg);
  if (goPayload.marker) {
    userMsg = goPayload.text || userMsg;
    if (goPayload.marker.cid) {
      const attribution = await loadMetaAttribution(env, goPayload.marker);
      await saveMetaAttribution(env, uid, attribution).catch((e) => {
        console.error('[pop-line] meta attribution', e.message);
      });
    }
  }
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/health') {
      ...
      return new Response(JSON.stringify({ ok: true, worker: 'pop-line-oa', seed: SEED_VER, ai_employee: AI_EMPLOYEE.display_name + '(' + AI_EMPLOYEE.job_title + ' ' + AI_EMPLOYEE.employee_id + ')', disclosure_mode: AI_EMPLOYEE.disclosure_mode, handoff_threshold: AI_EMPLOYEE.handoff_threshold, secret: !!cfg.lineSecret, token: !!cfg.lineToken, ai: cfg.anthropicKey ? 'claude-sonnet-4-6' : 'workers-ai-70b', owner: !!cfg.ownerId, crm: !!env.CRM, click_kv: !!env.CLICK_KV, evolved_insights: insights, delay_engine: env.DELAY_DO ? 'on' : 'off', ab_test: env.AB_TEST || 'on', quiet_mode: env.QUIET_MODE || 'on', read_delay: env.READ_DELAY || 'on', genome: !!env.GENOME, stickers: 'on', vision: 'on', deliveries, dbg: { last_post: lp || null, last_badsig: lb || null, last_oksig: lo || null } }), { headers: { 'Content-Type': 'application/json' } });
    }
