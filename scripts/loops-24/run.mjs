...
async function writeDashboard(result, candidates, autoCompletions = []) {
  ...
  const summary = {
    ...
    approvalWorkbenchRequestedExpiresAt: approvalWorkbench.available ? approvalWorkbench.summary?.requestedExpiresAt : null,
...
function renderMaterialFactory(data) {
  ...
  return [
    ...
    `- reused_pack: ${summary.reusedPack === true}`,
...
function renderApprovalWorkbench(data) {
  ...
  return [
    ...
    `- expiry_bound_reason: ${summary.expiryBoundReason || '(none)'}`,
    `- wakeup_fresh_until: ${summary.wakeupFreshUntil || '(unknown)'}`,
